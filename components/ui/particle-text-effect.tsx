...
class Particle {
  ...
  draw(ctx: CanvasRenderingContext2D, drawAsPoints: boolean, glow: boolean = false) {
...
interface ParticleTextEffectProps {
  ...
  className?: string;
  showCaption?: boolean;
  /** When true, canvas fills its parent container and has no border/box */
  fullscreen?: boolean;
  /** Custom particle colors as {r,g,b} — random if not provided */
  colors?: Array<{ r: number; g: number; b: number }>;
  /** Font size override; if not set, auto-scales based on canvas size */
  fontSize?: number;
  /** Interval in frames between word changes (default 240 = ~4s at 60fps) */
  changeInterval?: number;
}

const DEFAULT_WORDS = ["ДОХИО", "ТОГЛООМ", "ASL", "ОНОО", "ДАВТ"];

const THEME_COLORS: Array<{ r: number; g: number; b: number }> = [
...
export function ParticleTextEffect({
  ...
  const getNextColor = useCallback(() => {
    const palette = colors || THEME_COLORS;
  ...
  useEffect(() => {
    ...
    const nextWord = (word: string) => {
      ...
      // Auto-scale font size based on canvas width
      const computedFontSize = fontSize || Math.min(canvas.width / 6, canvas.height / 2.5, 140);

      offscreenCtx.fillStyle = "white";
      offscreenCtx.font = `bold ${computedFontSize}px Arial`;
    ...
    const animate = () => {
      ...
      for (let i = particles.length - 1; i >= 0; i--) {
        ...
        particle.move();
        particle.draw(ctx, drawAsPoints, fullscreen);
  ...
  }, [words, fontSize, changeInterval, getNextColor, fullscreen]);
  ...
  return (
    <div ref={containerRef} className={`flex flex-col items-center justify-center bg-black p-4 ${className}`}>
      <canvas
        ref={canvasRef}
